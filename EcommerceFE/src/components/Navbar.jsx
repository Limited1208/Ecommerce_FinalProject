...
export default function Navbar({ cartCount, onCartOpen, onLogoClick, onSearch }) {
    ...
    const selectSuggestion = (product) => { setQuery(product.name); setFocused(false); onSearch?.(product.name); };
    ...
    return (
        <nav className="sticky top-0 z-30 backdrop-blur-md" style={{ background: "rgba(13,8,0,0.95)", borderBottom: "1px solid #1e1000" }}>
            <style>{`
        .search-bar { width:0; opacity:0; padding:0; transition: width 0.35s cubic-bezier(0.22,1,0.36,1), opacity 0.25s ease, padding 0.3s ease; pointer-events:none; }
      ...
      `}</style>
            ...
            <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
                ...
                <button onClick={onLogoClick} className="flex items-center gap-2 hover:opacity-80 transition-opacity flex-shrink-0">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="url(#fireGrad)">
                        <defs>
                ...
                <div className="hidden md:flex items-center gap-8 text-xs tracking-widest uppercase"
                    style={{ color: "#664433", opacity: expanded ? 0.3 : 1, transition: "opacity 0.2s ease", pointerEvents: expanded ? "none" : "auto" }}
                >
                    {["New Drops", "Running", "Apparel", "Sale"].map((label) => (
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative" ref={wrapperRef}>
                        ...
                        >
                            <button onClick={expanded ? closeSearch : openSearch}
                            ...
                            <input
                                ...
                                onChange={handleChange} onKeyDown={handleKeyDown} onFocus={() => setFocused(true)}
                        ...
                        {/* Dropdown */}
                        {showDropdown && (
                            <div className="dropdown-enter absolute right-0 top-[calc(100%+10px)] w-80 rounded-xl overflow-hidden z-50"
                                    ...
                                    <>
                                        ...
                                        <ul>
                                            {suggestions.map((product, i) => (
                                                <li key={product.id}>
                                                    <button onMouseDown={() => selectSuggestion(product)} onMouseEnter={() => setActiveIdx(i)}
                                                        className="w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors"
                                                        style={{ background: activeIdx === i ? "#1e1000" : "transparent" }}
                                                    >
                                                        <img src={product.image} alt={product.name} className="w-10 h-10 object-cover rounded-lg flex-shrink-0" style={{ border: "1px solid #2a1500" }} />
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-[13px] text-white leading-snug truncate"><Highlight text={product.name} query={query} /></p>
                                                            <p className="text-[11px] mt-0.5" style={{ color: "#664433" }}>
                                                                <Highlight text={product.category} query={query} /> · <Highlight text={product.variant} query={query} />
                                                            </p>
                                                        </div>
                                                        <p className="heading text-[14px] text-[#ff6b00] flex-shrink-0">${product.price}</p>
                                                    </button>
                                                    {i < suggestions.length - 1 && <div className="mx-3 h-px" style={{ background: "#1e1000" }} />}
                                                </li>
                                            ))}
                                        </ul>
                                        <div className="px-4 py-2.5" style={{ borderTop: "1px solid #1e1000" }}>
                                            <p className="text-[10px]" style={{ color: "#2a1500" }}>↑↓ navigate · Enter to select · Esc to close</p>
                                        </div>
                                    </>
                                ) : (
                                    <div className="px-5 py-8 text-center">
                                        <p className="text-2xl mb-2">🔍</p>
                                        <p className="text-sm" style={{ color: "#664433" }}>No results for <span className="text-white font-medium">"{query}"</span></p>
                                    </div>
                                )}
                            </div>
                        )}
                    ...
                    >
                        ...
                        Cart
                        {cartCount > 0 && (
                            <span className="cart-badge w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center -ml-1"
